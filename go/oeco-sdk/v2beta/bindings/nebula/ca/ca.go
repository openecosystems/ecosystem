...
type Binding struct {
	...
	cp  *sdkv2betalib.CredentialProvider
...
func (aac *AccountAuthorityCache) Get(keyType AccountAuthorityKeyType, cp *sdkv2betalib.CredentialProvider, ecosystem string) (*CredStore, bool, error) {
	// Check if key exists
	...
	tempFile, err := os.CreateTemp(sdkv2betalib.CredentialDirectory, ".aa-"+key+"-*")
	if err != nil {
		return nil, false, sdkv2betalib.ErrServerInternal.WithInternalErrorDetail(errors.New("ca: cannot create temp directory"), err)
	}

	cred, err := cp.GetCredential(typev2pb.CredentialType_CREDENTIAL_TYPE_ACCOUNT_AUTHORITY, ecosystem)
	if err != nil {
		return nil, false, sdkv2betalib.ErrServerInternal.WithInternalErrorDetail(errors.New("nebula ca: failed to get credential"), err)
	}
...
func (b *Binding) GetAccountAuthority(_ context.Context, req *iamv2alphapb.CreateAccountAuthorityRequest) (*iamv2alphapb.AccountAuthority, error) {
	...
	if req.Name == "" {
		return nil, sdkv2betalib.ErrServerPreconditionFailed.WithInternalErrorDetail(errors.New("required parameter name is missing"))
	}
	...
	if err != nil {
		return nil, sdkv2betalib.ErrServerInternal.WithInternalErrorDetail(errors.New("failed to create temp file"), err)
	}
	...
	if err3 != nil {
		return nil, sdkv2betalib.ErrServerInternal.WithInternalErrorDetail(err3)
	}

	kfile, err4 := getFile(caKeyName, keyExt, keypath, now)
	if err4 != nil {
		return nil, sdkv2betalib.ErrServerInternal.WithInternalErrorDetail(err4)
	}

	qfile, err5 := getFile(caQrName, pngExt, qpath, now)
	if err5 != nil {
		return nil, sdkv2betalib.ErrServerInternal.WithInternalErrorDetail(err5)
	}
...
func (b *Binding) GetPKI(_ context.Context, req *iamv2alphapb.CreateAccountRequest) (cert *typev2pb.File, key *typev2pb.File, err error) {
	...
	if err != nil {
		return nil, nil, sdkv2betalib.ErrServerInternal.WithInternalErrorDetail(errors.New("failed to create temp file"), err)
	}
	defer os.RemoveAll(tempDir) //nolint:errcheck
	...
	if err3 != nil {
		return nil, nil, sdkv2betalib.ErrServerInternal.WithInternalErrorDetail(err3)
	}

	kfile, err4 := getFile(hostKeyName, keyExt, hostkeypath, now)
	if err4 != nil {
		return nil, nil, sdkv2betalib.ErrServerInternal.WithInternalErrorDetail(err4)
	}
...
func (b *Binding) SignCert(_ context.Context, req *iamv2alphapb.SignAccountRequest, opts ...CAOption) (*typev2pb.Credential, error) {
	...
	if err != nil {
		return nil, sdkv2betalib.ErrServerInternal.WithInternalErrorDetail(errors.New("failed to save unsigned cert file temporarily"), err)
	}

	ip, ipCIDR, err := b.getAnAvailableIPAddress(req, options)
	if err != nil {
		return nil, sdkv2betalib.ErrServerInternal.WithInternalErrorDetail(errors.New("failed to get an available IP address"), err)
	}

	hostname, err := b.getAvailableHostname(req, options)
	if err != nil {
		return nil, sdkv2betalib.ErrServerInternal.WithInternalErrorDetail(errors.New("failed to parse a hostname"), err)
	}
	...
	if err != nil {
		return nil, sdkv2betalib.ErrServerInternal.WithInternalErrorDetail(errors.New("error getting cert file"), err)
	}

	qrfile, err := getFile(signedQrName, pngExt, signedqrpath, now)
	if err != nil {
		return nil, sdkv2betalib.ErrServerInternal.WithInternalErrorDetail(errors.New("error getting qr code cert file"), err)
	}
...
func getFile(name string, extensionWithPeriod string, path string, time *timestamppb.Timestamp) (*typev2pb.File, error) {
	...
	if err != nil {
		return nil, sdkv2betalib.ErrServerInternal.WithInternalErrorDetail(errors.New("failed to read file: "+path), err)
	}
	...
	return &typev2pb.File{
		...
		Content:          data,
		Size:             int64(len(data)),
		Type:             sdkv2betalib.MimeTypeRegistry.GetMimeType(extensionWithPeriod),
...
func saveFileTemporarily(tempFolder string, file *typev2pb.File) (string, error) {
	...
	if err != nil {
		return "", sdkv2betalib.ErrServerInternal.WithInternalErrorDetail(errors.New("failed to save cert temporarily in : "+tempFolder), err)
	}
...
func WithCIDR(cidr string) CAOption {
	return optionFunc(func(cfg *caOptions) {
		c, err := sdkv2betalib.NewCIDR(cidr)
		if err != nil {
			return
		}
