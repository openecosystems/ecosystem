...
import (
	...
	"libs/public/go/model/gen/platform/system/v2alpha"
...
func (s *SystemServiceHandler) Enable(ctx context.Context, req *connect.Request[systemv2alphapb.EnableRequest]) (*connect.Response[systemv2alphapb.EnableResponse], error) {
	...
	if spec.SpecData.FieldMask != nil && len(spec.SpecData.FieldMask.Paths) > 0 {
		...
		if !spec.SpecData.FieldMask.IsValid(&systemv2alphapb.EnableResponse{}) {
	...
	var dd systemv2alphapb.EnableResponse
...
func (s *SystemServiceHandler) Disable(ctx context.Context, req *connect.Request[systemv2alphapb.DisableRequest]) (*connect.Response[systemv2alphapb.DisableResponse], error) {
	...
	if spec.SpecData.FieldMask != nil && len(spec.SpecData.FieldMask.Paths) > 0 {
		spec.SpecData.FieldMask.Normalize()
		if !spec.SpecData.FieldMask.IsValid(&systemv2alphapb.DisableResponse{}) {
			log.Error("Invalid field mask")
			return nil, sdkv2alphalib.ErrServerPreconditionFailed.WithInternalErrorDetail(errors.New("Invalid field mask"))
		}
	}
	...
	var dd systemv2alphapb.DisableResponse
	err3 := proto.Unmarshal(reply.Data, &dd)
	if err3 != nil {
		log.Error(err3.Error())
		return nil, connect.NewError(connect.CodeInternal, errors.New("internal error"))
