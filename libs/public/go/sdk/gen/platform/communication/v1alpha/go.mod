// Code generated by protoc-gen-platform go/sdk. DO NOT EDIT.
// source: platform/communication/v1alpha/preference_center.proto
// sourcePath: libs/public/go/sdk/gen/communication/v1alpha
module github.com/openecosystems/ecosystem/libs/public/go/sdk/gen/platform/communication/v1alpha
go 1.23.4

require connectrpc.com/connect v1.17.0

require github.com/google/go-cmp v0.6.0 // indirect

require (
	golang.org/x/net v0.29.0 // indirect
	google.golang.org/protobuf v1.34.2 // indirect
)
