...
type TypeScriptSpecModule struct {
	*pgs.ModuleBase
	ctx pgsgo.Context
	Tpl *template.Template
}

func TypeScriptSpecPlugin() *TypeScriptSpecModule {
	return &TypeScriptSpecModule{ModuleBase: &pgs.ModuleBase{}}
}

func (m *TypeScriptSpecModule) InitContext(c pgs.BuildContext) {
	m.ModuleBase.InitContext(c)
	m.ctx = pgsgo.InitContext(c.Parameters())
}

func (m *TypeScriptSpecModule) Name() string { return language + "/" + pluginName }

func (m *TypeScriptSpecModule) Execute(targets map[string]pgs.File, _ map[string]pgs.Package) []pgs.Artifact {
...
func (m TypeScriptSpecModule) GenerateFile(file pgs.File) {
