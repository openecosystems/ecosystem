...
`,
	Run: func(cmd *cobra.Command, _ []string) {
		...
		client := *NewConfigurationServiceSpecClient(sdkv2alphalib.Config, sdkv2alphalib.Config.Platform.Endpoint, connect.WithInterceptors(sdkv2alphalib.NewCLIInterceptor(sdkv2alphalib.Config, sdkv2alphalib.Overrides)))
		response, err := client.CreateConfiguration(context.Background(), request)
		if err != nil {
			fmt.Println("Error with getting response: " + err.Error())
			os.Exit(1)
		}
