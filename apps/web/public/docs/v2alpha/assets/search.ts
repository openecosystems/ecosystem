      ...
      <div class="ais-RefinementList">
        <ul class="ais-RefinementList-list">
          ${items
              .map(
                  (item) => `
          <li class="ais-RefinementList-item${item.isRefined ? ' ais-RefinementList-item--selected' : ''}">
            <div>
              <label class="ais-RefinementList-label" data-value="${item.value}">
                <input type="checkbox" class="ais-RefinementList-checkbox" ${item.isRefined ? 'checked ' : ''}value="${
                      item.label
                  }">
...
(function () {
    ...
    const virtualSearchBox = instantsearch.connectors.connectSearchBox(() => {
        const inputBox = (document.getElementById('autocomplete-0-input') as HTMLInputElement) || null;
        if (inputBox) {
            ...
            <article>
              <h1>
                <a class="ProductGrid--link" href="${hit.url}">
                  ${typeof hit.type === 'string' && hit.type === 'system' ? `<div>${hit.logo}</div>` : ''}
                  ${hit.title}
                </a>
              </h1>
